...
export default function ShowroomPage() {
    ...
    return (
        <div className="min-h-screen bg-[#020617] text-white">
            ...
            <div className="relative z-10 grid lg:grid-cols-[1fr,400px] h-screen pt-20">

                {/* 3D Viewport - Now Dynamic */}
                <div className="relative h-[50vh] lg:h-full w-full border-r border-white/5">
                    {/* Overlay Information Dynamic */}
                    <div className="absolute top-6 left-6 z-20 pointer-events-none">
                        <Badge className="bg-blue-600/80 backdrop-blur-md text-white border-none px-3 py-1 mb-3 pointer-events-auto">
                            {selectedCar.year} Model
                        </Badge>
                        <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter drop-shadow-2xl">
                            {selectedCar.brand}
                            <span className="block text-blue-500 text-3xl md:text-5xl mt-1">{selectedCar.model}</span>
                        </h1>
                    </div>

                    {/* Stats Dynamic Overlap */}
                    <div className="absolute bottom-6 left-6 z-20 hidden md:flex gap-4 pointer-events-none">
                        <div className="bg-black/40 backdrop-blur-md rounded-xl p-4 border border-white/5 flex gap-6 text-xs font-mono text-slate-300 pointer-events-auto">
                            <div>
                ...
                {/* Controls Sidebar */}
                <div className="bg-[#0f172a]/95 backdrop-blur-xl border-l border-white/5 p-6 md:p-8 flex flex-col h-full overflow-y-auto w-full">
                    ...
                    <div className="mb-8">
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Select Finish</h3>
                    ...
                    <div className="mt-auto space-y-4">
                        ...
                        <Button className="w-full h-14 bg-blue-600 hover:bg-blue-500 text-lg font-bold rounded-xl shadow-lg shadow-blue-600/20">
                            Book Now <span className="ml-2 opacity-80 text-sm">from ${selectedCar.price_per_day}/day</span>
                            <ArrowRight className="ml-2 h-5 w-5" />
                        </Button>
                        <Button variant="outline" className="w-full h-12 border-white/10 text-slate-300 hover:bg-white/5 hover:text-white rounded-xl">
